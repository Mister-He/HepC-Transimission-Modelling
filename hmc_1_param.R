  ...
  summarise_obs_ci <- function(ppc_mat, obs, type_label) {
    ...
    if (type_label == "HCV-positive") {
      ...
      sd_vals <- vapply(seq_len(ncol(q_mat)), function(j) {
        ...
        if (!length(q_draws)) return(NA_real_)
        stats::median(
      ...
      sd_vals <- vapply(seq_len(ncol(p_mat)), function(j) {
        ...
        if (!length(p_draws)) return(NA_real_)
...
N_WARMUP    <- 100L    # adaptation (discarded)
N_ITER      <- 400L   # total iterations per chain  (post-warmup = N_ITER - N_WARMUP)
...
inits <- lapply(seq_len(N_CHAINS), function(ch) {
  c(
    ...
    log(runif(1L, 0.01, 1.0))       # log(C_contact_scale):  prior median 1, start positive
  )
})
